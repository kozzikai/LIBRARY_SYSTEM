...
def pay():
    try:
        ...
        logger.info(
            f"Updation of completed payment on loan ids {','.join(loan_ids)} failed with exception: {e}"
        )
    ...
    logger.info(
        f"Fine payment on loan ids {','.join(loan_ids)} is successfully completed"
    )
...
def destroy_button(name):
    ...
    try:
        logger.info(
            f"Trying to remove previously loaded {name} button if they exist")
        buttons[name].destroy()
    except Exception as e:
        logger.info(f"No previously loaded {name} button to remove")


def update_back_button(text, background, command, x_pos, y_pos, rel_width,
                       rel_height):
    global back_button
...
def display_fine_by_card_id_new():
    ...
    if not is_valid_card_id(card_id):
        update_back_button("BACK", 'snow', master.destroy, 0.40, 0.90, 0.18,
                           0.05)
        logger.info(
            f"Card_id {card_id} doesn't exist. Please enter a valid card id")
    ...
    if not existing_fine_count:
        ...
        messagebox.showinfo("FAILURE",
                            f"No existing fines for card_id {card_id}")
        return
    ...
    # Define the columns
    tree['columns'] = ('LOAN_ID', 'ISBN', 'CARD_ID', 'DATE_OUT', 'DUE_DATE',
                       'DATE_IN', 'FINE_AMOUNT', 'PAID')
    ...
    for fine_entry in fines_list:
        ...
        tree.insert('',
                    END,
                    values=(fine_entry["Loan_id"], fine_entry["Isbn"],
                            fine_entry["Card_id"], fine_entry["Date_out"],
                            fine_entry["Due_date"], fine_entry["Date_in"],
    ...
    if existing_loans > 0:
        ...
        messagebox.showerror(
            "FAILURE",
            "Please return back books that crossed due date before paying the fine"
        )
        return

    buttons["pay_fine_button"] = create_button(master,
                                               text=f"Pay Fine: {fine_amount}",
                                               background='#d1ccc0',
                                               foreground='black',
                                               command=pay,
                                               x_pos=0.28,
...
def load_card_number_page():
    ...
    # Issue Button
    get_fine_button = create_button(master,
                                    text="FIND",
                                    background='snow',
                                    foreground='black',
                                    command=display_fine_by_card_id_new,
                                    x_pos=0.80,
                                    y_pos=0.05,
                                    rel_width=0.18,
                                    rel_height=0.05)
