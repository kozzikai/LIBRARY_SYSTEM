...
def issue():
    ...
    if not is_valid_card_id(card_id):
        messagebox.showerror(
            ...
            f"Card_id doesn't exist. Please enter a valid card id")
        return
    ...
    if (existing_loans + len(selected_indices) <= 3):
        ...
        if due_date_loans > 0:
            logger.error(
                f"There are existing loans on Card id {card_id} that crossed due date"
            )
            messagebox.showerror(
                "FAILURE",
        ...
        cur.execute("SELECT DATE_ADD('" + str(today_date) +
                    "', INTERVAL 14 DAY)")
        ...
        try:
            for index in selected_indices:
                ...
                logger.info(
                    f"Issuing book with isbn {isbn} on Card id {card_id}")
                cur.execute(
                    f"INSERT INTO {LOANS_TABLE}(Isbn,Card_id,Date_out,Due_date) VALUES(%s,%s,%s,%s)",
                    (str(isbn), str(card_id), str(today_date), str(due_date)))
                cur.execute(
                    f"SELECT * FROM {LOANS_TABLE} WHERE Isbn = '{str(isbn)}' AND Date_in IS NULL"
                )
                loan_id = cur.fetchone()[0]
                logger.info(
                    f"Loan id is: {loan_id}, Current Date: {today_date}, Due date: {due_date}"
                )

                cur.execute(f"INSERT INTO {FINES_TABLE}(Loan_id) VALUES(" +
                            str(loan_id) + ")")
                issued.append(str(isbn))
            con.commit()
            logger.info(f"Succesfully Issued Books: {','.join(issued)}")
            messagebox.showinfo(
                "Success", f"Succesfully Issued Books: {','.join(issued)}")
        ...
        if existing_loans == 3:
            logger.error(
                "Cannot Issue New Books. Maximum No. of Book Loans for a borrower is 3"
            )
            messagebox.showerror(
                "Error",
                "Cannot Issue New Books. Maximum No. of Book Loans for a borrower is 3"
            ...
            logger.error(
                f"Maximum No. of Book Loans for a borrower is 3. Please select {3 - existing_loans} or less books"
            )
            messagebox.showerror(
                "Error",
                f"Maximum No. of Book Loans for a borrower is 3. Please select {3 - existing_loans} or less books"
...
def get_card_number_page():
    ...
    card_id_label, card_id_entry = create_label_entry(master,
                                                      text="CARD ID: ",
                                                      background='burlywood3',
                                                      foreground='tan4',
                                                      label_x_pos=0.05,
    ...
    # Issue Button
    issue_button = create_button(master,
                                 text="Issue",
                                 background='#d1ccc0',
                                 foreground='black',
                                 command=issue,
    ...
    exit_button = create_button(master,
                                text="Exit",
                                background='#aaa69d',
                                foreground='black',
...
def on_select(event):
    ...
    logger.info(
        f"on_select event triggered on selecting index {tree.index(last_selected_id)}"
    )
    if last_selected_id:
    ...
    if availability.lower() == "no":
        tree.selection_remove(last_selected_id)
        logger.info(
            f"Selected book index {tree.index(last_selected_id)} is unavailable"
        )
    ...
    if len(selected_ids) > 3:
        ...
        messagebox.showwarning("Warning",
                               "Cannot select more than 3 entries at a time")
...
def search_by_attribute(search_query):
    ...
    for entry in matched_entries:
        ...
        tree.insert('',
                    tk.END,
                    values=((f"{serial_no}", isbn, title, authors, available)))
...
def search():
    ...
    # Creating a treeview
    tree = ttk.Treeview(buttons["search_bar_frame"],
                        show='headings',
                        selectmode='none')
    ...
    logger.info(
        f"No. of book entries matching the given search: {len(result_list)}")
    buttons["search_bar_frame"].place(relx=0.02,
                                      rely=0.15,
                                      relwidth=0.96,
                                      relheight=0.7)

    # Add vertical scrollbar
    vscroll = ttk.Scrollbar(buttons["search_bar_frame"],
                            orient="vertical",
                            command=tree.yview)
    ...
    hscroll = ttk.Scrollbar(buttons["search_bar_frame"],
                            orient="horizontal",
                            command=tree.xview)
    ...
    if not tree.get_children():
        ...
        messagebox.showinfo(
            "INFO", "No books with matching Isbn/Title/Author_Names Pattern")
        return

    buttons["issue_button"] = create_button(master,
                                            text="ISSUE",
                                            background='black',
                                            foreground='#333945',
                                            command=get_card_number_page,
                                            x_pos=0.30,
                                            y_pos=0.9,
                                            rel_width=0.18,
                                            rel_height=0.05)
