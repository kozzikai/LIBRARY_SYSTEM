...
def employee_menu(master, canvas):
    ...
    # Add Borrowers
    add_borrower_btn = Button(master, text="ADD BORROWER", bg='snow', fg='black', command=get_borrower_data)
    ...
    # Display Borrowers
    add_borrower_btn = Button(master, text="DISPLAY BORROWER DETAILS", bg='snow', fg='black', command=borrowers_search_page)
    add_borrower_btn .config(borderwidth=0, relief="flat", highlightthickness=0)
    add_borrower_btn.place(relx=0.28, rely=0.35, relwidth=0.45, relheight=0.1)

    # Search Book Button
    search_book_btn = Button(master, text="CHECK-OUT BOOK", bg='snow', fg='black', command=load_search_page)
    ...
    checkin_book_btn = Button(master, text="CHECK-IN BOOK", bg='snow', fg='black', command=loans_search_page)
    ...
    refresh_fines_btn = Button(master, text="REFRESH FINES", bg='snow', fg='black', command=display_fines)
    refresh_fines_btn .config(borderwidth=0, relief="flat", highlightthickness=0)
    refresh_fines_btn.place(relx=0.28, rely=0.65, relwidth=0.45, relheight=0.1)

    # Pay Fine Button
    pay_fine_btn = Button(master, text="PAY FINE", bg='snow', fg='black', command=load_card_number_page)
